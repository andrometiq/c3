...
Display the output verbatim. `c3-broker update` checks the latest GitHub release, downloads the tarball for this platform, verifies it against `SHA256SUMS`, validates all nine shipped binaries, and replaces the eight core binaries in place. It refreshes `codex` only when the existing destination is positively identified as C3's already-installed launcher, so update cannot opt a user into Codex integration. It does **not** touch the running broker process — the replacement is on disk only. On Windows, installation is refused; fully quit C3 and re-extract the release tarball manually.
