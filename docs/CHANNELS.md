...
- **Persisted-offset advance**: the read offset advances only to the highest contiguous `update_id` whose message has been durably persisted (`fsync`'d) to the inbound queue, or which was a no-op (gated, `/status`, or a non-message update). A voice row satisfies that boundary as soon as its honest pending placeholder is `fsync`'d; STT is later enrichment of that durable row, and startup scanning recovers unfinished rows after a crash. With the queue disabled, routed inbound holds the Telegram offset for replay after restart with a working queue; anything delivered live will arrive again, subject to the provider's retention. Storage is per-message, and debounce/merge is a delivery-presentation concern that does not merge stored lines. See `docs/USAGE.md` "Durable inbound queue & backlog".
