...
| `attach`        | `attach(expr=…)` (MCP tool)       | LLM dispatch  | Attach this session's adapter to a Telegram topic. Optional `bot=` selects a Swarm extra bot. Broker parses `expr` and either claims an explicit target, resumes the session's own topic, or proposes a picker/confirmation. |
...
| `/mute` | any allowlisted sender, **Swarm only** | Stop this bot listening in this topic until it is tagged again. The session stays attached. No-op when Swarm extra bots are not configured. |
