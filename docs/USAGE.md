...
- **Queue-disabled intake holds at Telegram.** If the queue cannot open at startup, the broker logs `durable inbound hold DISABLED for this run` and continues best-effort live delivery without advancing Telegram offsets. Inbound is replayed when the broker restarts with a working queue; anything delivered live in the meantime will arrive again. The earliest unpersisted update holds the global frontier across all topics, so later inbound can wait too.
...
The fix is to make the queue directory (`$XDG_STATE_HOME/c3/queue/`, fallback `~/.local/state/c3/queue/`) writable and restart the broker. Live delivery continues best-effort; replay after restart includes messages already delivered live.
