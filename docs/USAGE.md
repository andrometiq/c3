...
- **Voice notes** — record on your phone, send to the topic. C3 persists a pending row immediately, then the bounded STT scheduler transcribes off the route worker; the CLI sees `[Transcribed voice]: <text>` plus the original voice attachment available for re-download if the transcript is wrong.
...
- **Held-count auto-reply.** After scheduling, rows still waiting get `📨 Held — nothing lost. 1 message queued. Send /status to check.` (or `2 messages queued`). Telegram sends one reply per backlog episode, quoting a real local inbound; drained rows are never quoted. The episode ends when the durable backlog empties; restarting the broker can produce one extra notice. Pending voice work suppresses the ordinary Held notice: its quoted transcript or failure readback carries a short count such as `📨 1 held in queue.` when backlog remains. Counts exclude open live attempts, open fetch groups and observed receipts and are recomputed at send time. Failed sends and unreadable snapshots remain retryable. Web keeps its status-event flow and 10-second cooldown.
- **Delivery status.** Automatic route diagnostics are not sent to chat. `/status` describes delivery availability and queue recovery; local broker status and the MCP preamble retain detailed route diagnostics.
...
Without a local queue, inbound is held at Telegram for replay when the broker restarts with a working queue; anything delivered live in the meantime will arrive again. C3 reports this on each operator surface:
...
- **In `c3-broker status`** — the same persistent `durable queue: DISABLED` warning appears in the one-shot shell summary for as long as this broker is running without its queue.
- **On every hold** — the auto-reply becomes *"⚠️ Held at Telegram — local queue unavailable."* instead of *"📨 Held — nothing lost."* It is deliberately on the slower 5-minute-per-topic cooldown independently of the ordinary Held episode: the warning is the same sentence every time, and repeating it through a burst is how an alert gets ignored.
- **In `broker.log`** — the `durable inbound hold DISABLED for this run` line at startup, then a `HELD AT TELEGRAM — durable queue disabled` line for each message without a live handoff, carrying the sender and the text (truncated to 200 characters).

The fix is to make the queue directory (`$XDG_STATE_HOME/c3/queue/`, fallback `~/.local/state/c3/queue/`) writable and restart the broker, within Telegram's 24-hour update retention. Live delivery continues best-effort; replay after restart includes messages already delivered live.
...
`retranscribe(file_id)` joins the same bounded, durable scheduler as automatic transcription and still returns the fresh transcript synchronously. Pass the optional `message_id`: if that message has a pending voice row, the scheduler resolves it in place; if the row was already consumed, is already final, or is absent, C3 appends an explicit `[transcript update for voice message …]` row. A caller timeout or disconnect does not cancel the durable resolve or its delivery. The original attachment remains available, so a failed transcription does not require re-recording.
...
global summary. Telegram status reads wait for the route worker to finish any queue rewrite;
the reply arrives asynchronously so polling can continue. “Oldest” is the minimum
timestamp among waiting rows, including older drain imports at the tail.
Negotiated route history says `was inbox, confirmed <age>` after
