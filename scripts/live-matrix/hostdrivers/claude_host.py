...
from hostdriver import remaining_timeout
from evidence_io import checked_read, read_jsonl, read_problem
...
class Host:
    def __init__(self, root, claude, broker, adapter, scripts, cell, version, timeout=90, *, authentication_reference=None):
        ...
        original = authentication_reference or Path(os.environ.get("CLAUDE_CONFIG_DIR", str(Path.home() / ".claude")))
        ...
        for args in (("plugin", "marketplace", "add", str(market)), ("plugin", "install", "c3@c3", "--scope", "user")):
            with (self.control / "plugin-setup.log").open("ab") as out:
                subprocess.run([str(claude), *args], env=self.env, cwd=self.cwd, stdout=out, stderr=out, timeout=remaining_timeout(timeout), check=True)
    ...
    def tmux(self, *args, check=True):
        return subprocess.run(["tmux", "-S", str(self.socket), *args], env=self.env,
                              stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=check, timeout=remaining_timeout(getattr(self, 'command_timeout', 15)))
    ...
    def send(self, text):
        ...
        if composer is None:
            raise HostSetupError("unrecognized composer; submission uncertain; see control/pane.txt")
        if flatten(text) not in flatten(composer):
            raise HostSetupError("prompt not observed in composer; submission uncertain; see control/pane.txt")
        self.submission_composer_before = composer
        for attempt in range(5):
            ...
            for _ in range(6):
                ...
                if composer is None:
                    raise HostSetupError("unrecognized composer; submission uncertain; see control/pane.txt")
                if probe not in flatten(composer):
                    self.submission_composer = composer
                    return
    ...
    def transcript(self):
        ...
        if not path.is_absolute() or not path.resolve().is_relative_to(self.root.resolve()):
    ...
    def stop_session(self):
        ...
        wait_for(lambda: self.tmux("has-session", "-t", "matrix", check=False).returncode != 0,
                 getattr(self, 'exit_timeout', 15), "Claude did not exit")
    ...
    def reconnect(self, keys=None):
        stamp = time.time()
        self.send("/mcp")
        wait_for(lambda: "Manage MCP servers" in self.pane(), getattr(self, 'menu_timeout', 15),
        ...
        if keys:
            ...
            def menu_number():
                ...
                return None
            number = wait_for(menu_number, getattr(self, 'server_menu_timeout', 10), "unrecognized /mcp server view; supply --reconnect-keys for this version")
