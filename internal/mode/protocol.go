...
const ModeProtocol = "OUTPUT MODE PROTOCOL (per-session, agent-only state — default CLI mode on every fresh session):\n" +
	"• CLI mode (DEFAULT): your replies go to the CLI terminal. Held channel routes are INPUT-ONLY — voice and replies from them arrive as `<channel>` blocks. DO NOT call the `reply` tool to respond unless the user explicitly asks.\n" +
	"• Telegram mode and Drive mode are OUTPUT-ROUTE choices: Telegram mode sends substantive `reply` calls to the held Telegram output route; Drive mode sends them to the held web output route. Switch ONLY when the user EXPLICITLY asks (e.g. \"switch to Telegram\" / \"Telegram mode\", \"switch to the web chat\" / \"Drive mode\", or \"switch to CLI\" / \"CLI mode\"). NEVER auto-switch by inferring where the user is — a message arriving from Telegram or web is NOT a switch request, and neither is 'stepping away from the laptop'. If you think a switch is warranted, ASK and get explicit confirmation first; do not switch unilaterally.\n" +
	"• When you hold more than one route, `reply` goes to your OUTPUT route (Telegram mode → the Telegram route; Drive mode → the web route). To answer on another route you hold for a single message, pass `channel=<that route>` on `reply`. Move the output route itself only on an explicit request, with the `output` tool.\n" +
	"• The CLI/Telegram/Drive mode is your responsibility to track. The broker stores only the held route set and output route. Always start in CLI mode; honor the user's EXPLICIT switch instructions immediately, but never switch on your own.\n" +
	"• After attach completes, briefly announce your current output mode (\"currently in CLI mode\" / \"currently in Telegram mode\" / \"currently in Drive mode\") so the human has explicit confirmation of where replies will land."

// OnTheGoProtocol defines the web-chat trigger phrases as explicit output-mode
// requests while retaining the Telegram topic as a held input route.
const OnTheGoProtocol = "ON-THE-GO MODE (phone, web chat): infer nothing from message origin. Only when the user explicitly says \"start on-the-go mode\" / \"switch to the web chat\", call attach with expr \"+web\". This ADDS web, KEEPS the Telegram topic held for input, and makes web the output route (Drive mode); announce it in one line, and follow the SPOKEN REPLIES guidance whenever a \"Spoken replies ON\" notice is in effect. On the web page an HTML file you send opens in a sandboxed viewer inside the page: generate self-contained HTML (inline CSS/JS, `data:` images, no network fetches — they are blocked), write it to a file and send it as `media: [{kind:\"file\", path, caption}]`; never put secrets in it. Only when the user explicitly says \"end on-the-go mode\" / \"back to Telegram\" / \"back to the topic\", call detach with target \"web\"; this keeps the already-held Telegram topic and makes it the output route (Telegram mode), so there is nothing to re-attach. Announce the current mode. Permission prompts and `ask` are answered at the laptop throughout."
