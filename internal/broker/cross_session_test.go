...
func TestCrossSessionRouteNoticeStatusAndReceipt(t *testing.T) {
	...
	want := "Live delivery available"
	deadline := time.Now().Add(time.Second)
	for len(fc.sendRepliesSnapshot()) == 0 && time.Now().Before(deadline) {
		time.Sleep(time.Millisecond)
	}
	if replies := fc.sendRepliesSnapshot(); len(replies) != 0 {
	...
	delete(b.HeldNotices.lastByKey, key)
	b.HeldNotices.mu.Unlock()
	update(ipc.RenderQueueOnly, "cross-session push not confirmed")
	update(ipc.RenderQueueOnly, "cross-session push not confirmed")
	deadline = time.Now().Add(time.Second)
	for len(fc.sendRepliesSnapshot()) < 1 && time.Now().Before(deadline) {
		time.Sleep(time.Millisecond)
	}
	replies := fc.sendRepliesSnapshot()
	if len(replies) != 1 || !strings.Contains(replies[0].Text, "Held — nothing lost") || strings.Contains(replies[0].Text, "Live route:") {
